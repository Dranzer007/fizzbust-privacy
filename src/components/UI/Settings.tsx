  ...
  return (
    <div className="fixed inset-0 bg-white flex flex-col items-center p-4 sm:p-8 overflow-y-auto select-none">
      <motion.div
        ...
        className="w-full max-w-2xl relative z-10 pt-4 sm:pt-12 pb-8 sm:pb-24"
      >
        <div className="flex flex-col landscape:flex-row items-center landscape:items-end justify-between mb-12 sm:mb-24 gap-4 sm:gap-0">
          <button 
            onClick={() => {
              soundManager.play('tap');
              onBack();
        ...
        <div className="space-y-4 sm:space-y-6">
          {/* Music Toggle */}
          <div className="bg-black/5 p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2.5rem] flex items-center justify-between border border-black/5 group hover:bg-black/10 transition-all">
            ...
            <button
              ...
              className={`relative w-16 h-8 sm:w-20 sm:h-10 rounded-full transition-all duration-300 ${musicMuted ? 'bg-black/10' : 'bg-sunset-teal'}`}
            >
              <motion.div
                animate={{ x: musicMuted ? 4 : (window.innerWidth < 640 ? 36 : 44) }}
          ...
          <div className="bg-black/5 p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2.5rem] flex items-center justify-between border border-black/5 group hover:bg-black/10 transition-all">
            ...
            <button
              ...
              className={`relative w-16 h-8 sm:w-20 sm:h-10 rounded-full transition-all duration-300 ${sfxMuted ? 'bg-black/10' : 'bg-sunset-orange'}`}
            >
              <motion.div
                animate={{ x: sfxMuted ? 4 : (window.innerWidth < 640 ? 36 : 44) }}
                className="absolute top-1 w-6 h-6 sm:w-8 sm:h-8 bg-white rounded-full shadow-lg"
              />
            </button>
          </div>
          ...
          <div className="bg-black/5 p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2.5rem] flex items-center justify-between border border-black/5 group hover:bg-black/10 transition-all">
            ...
            <button
              ...
              className={`relative w-16 h-8 sm:w-20 sm:h-10 rounded-full transition-all duration-300 ${!hapticsEnabled ? 'bg-black/10' : 'bg-sunset-pink'}`}
            >
              <motion.div
                animate={{ x: !hapticsEnabled ? 4 : (window.innerWidth < 640 ? 36 : 44) }}
                className="absolute top-1 w-6 h-6 sm:w-8 sm:h-8 bg-white rounded-full shadow-lg"
              />
            </button>
          </div>
        ...
        <div className="mt-16 sm:mt-32 flex justify-between items-center opacity-20">
